...
@media screen and (max-width: 480px) {
  ...
  return (
    <div className="tracking-container" style={{marginTop:"-2rem"}}>
      ...
      >
        {isFormOpen ? <X size={24} /> : <Menu size={24} />}
      </button>
 ...
 {userRole === 'start' && (
        ...
        style={{
          '@media (max-width: 768px)': {
            position: isFormOpen ? 'fixed' : 'absolute',
            top: isFormOpen ? '0' : '115px',
            left: isFormOpen ? '0' : '55px',
            width: isFormOpen ? '100%' : '450px',
            height: isFormOpen ? '100%' : 'auto',
            zIndex: isFormOpen ? 1100 : 1000,
            backgroundColor: isFormOpen ? 'white' : 'transparent',
            overflowY: isFormOpen ? 'auto' : 'visible'
          }
        }}>
  ...
  <div className="overlay-modal-wrapper">
    <div className="modal-popup-container">
      ...
      {savedRoutes.length > 0 ? (
        <table className="saved-routes-table">
          <thead>
  <tr style={{backgroundColor: "#007bff" , color :"white ", borderRadius:"5px"}}>
